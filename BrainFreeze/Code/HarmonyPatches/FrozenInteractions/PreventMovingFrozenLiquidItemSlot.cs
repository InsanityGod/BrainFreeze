...
public static class PreventMovingFrozenLiquidItemSlot
{
    ...
    {
        var derivedTypes = AccessTools.AllTypes().Where(type => type != typeof(ItemSlotCreative) && type != typeof(ItemSlotPerPlayer) && typeof(ItemSlot).IsAssignableFrom(type));
        ...
        {
            var method1 = AccessTools.Method(type, "ActivateSlotRightClick", parameters: [typeof(ItemSlot), typeof(ItemStackMoveOperation).MakeByRefType()]);
            if (method1 != null) yield return method1;
            var method2 = AccessTools.Method(type, "ActivateSlotLeftClick", parameters: [typeof(ItemSlot), typeof(ItemStackMoveOperation).MakeByRefType()]);
